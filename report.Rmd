...
Primerament descarregarem les dades de les dues web anteriors:
...
Un cop descarregades les dades les obrirem amb el paquet `readr`:
...
En aquest punt, seleccionarem una data per treballar. Per tal de que cada alumne (o grup) treballi amb dates diferents, la farem dependre d'un codi d'estudiant (si sou un grup de dos, podeu posar el que preferiu):
...
Estem interessat en analitzar quina relació hi ha entre les decisions de govern (polítiques de tancament i contenció, i econòmiques), i com aquestes es relacionen amb la variació de casos a la població (`y`).

2. **Prepara el conjunt de dades `data` per a ser analitzat segons les característiques de les variables. Explica les decisions que prens i quina és la tipologia de les variables que tens.**
...
5. **Crea un model que expliqui el creixement observat a partir de les variables que creguis adients.**
